...
| #   | Lab                                                                                                                                              | Description                                                                                                                                                                                                                                                                       |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7   | [MeetingNoSession](https://github.com/leberns/opendev/blob/main/AgentsLab/src/Lab.MultiTurnConversation/MeetingNoSession.cs)                     | Demonstrates an agent without chat history. Because there is no history the agent cannot answer questions based on previous messages.                                                                                                                                             |
| 8   | [MeetingHistoryInMemory](https://github.com/leberns/opendev/blob/main/AgentsLab/src/Lab.MultiTurnConversation/MeetingHistoryInMemory.cs)         | Demonstrates an agent with chat history. The agent can answer questions based on previous messages, enabling multi-turn conversations. Because the history is stored in memory it is not shared between sessions or requests.                                                     |
| 9   | [MeetingHistoryCosmosDb](https://github.com/leberns/opendev/blob/main/AgentsLab/src/Lab.MultiTurnConversation/MeetingHistoryCosmosDb.cs)         | Demonstrates an agent with chat history stored in a database. Because the history is persisted in a database it can be shared between sessions or requests. Requires the Azure Cosmos DB infrastructure (see prerequisites).                                                      |
...
| #   | Lab                                                                                                                                     | Description                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 14  | [GuardrailMiddlewareAgent](https://github.com/leberns/opendev/blob/main/AgentsLab/src/Lab.MiddlewareAgents/GuardrailMiddlewareAgent.cs) | Demonstrates a guardrail middleware that filters out inappropriate content produced by a writer agent. The guardrail has two parts: a sensor agent detects inappropriate words and extracts them into a findings record, then a redaction algorithm replaces the findings by a "redacted" text. Note: the sensor agent can be replaced by a deterministic logic for well-known patterns (e.g. emails, phone numbers, or other sensitive data). |
...
| #   | Lab                                                                                                                        | Description                                                                                                                                                                                               |
| --- | -------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 18  | [McpSubmitExpenseAgent](https://github.com/leberns/opendev/blob/main/AgentsLab/src/Lab.McpAgents/McpSubmitExpenseAgent.cs) | Shows an agent calling a MCP service to create an expense. Architecture: `SubmitExpenseAgent → FinanceMCPClient → FinanceMCPServer → FinanceExpenseAPI`.                                                  |
| 19  | [McpServicesAgent](https://github.com/leberns/opendev/blob/main/AgentsLab/src/Lab.McpAgents/McpServicesAgent.cs)           | Shows an agent that can perform several services by calling the appropriate MCP servers (Finance and IT Support). The input is intentionally incomplete, so the agent asks the user for more information. |
| 20  | [McpServicesAgency](https://github.com/leberns/opendev/blob/main/AgentsLab/src/Lab.McpAgents/McpServicesAgency.cs)         | Shows a multi-agent architecture where a plan agent decomposes user requests and routes them to specialised Finance and IT Support agents, each backed by its own MCP server.                             |
